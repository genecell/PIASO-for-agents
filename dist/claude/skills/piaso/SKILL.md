---
name: piaso
description: PIASO single-cell omics ecosystem for scRNA-seq / spatial analysis. Use for: marker gene identification / find markers for clusters; ligand-receptor analysis / cell-cell communication; gene-set scoring / gene-set enrichment / score a gene set; cell type annotation / label transfer / predict cell types; marker-gene-guided dimensionality reduction (GDR); INFOG normalization; batch integration across samples; differential expression across conditions. Components (each a standalone trigger): PIASO, COSG, COSGR, LARIS, Emergene, SCALAR, PIASOmarkerDB. Covers Python (scanpy/AnnData) and R (Seurat/SCE, via COSGR). Fires on .h5ad, AnnData, Seurat, .rds, Visium, spatial, and on any of the component names even without 'PIASO'. Includes the single-cell (SCALAR) vs spatial (LARIS) ligand-receptor choice.
